...
if [ ! -e "${dir}/wgs/GC_bias/${sample}.GC_bias.txt" ]; then
    bedtools intersect -a ${bed} -b ${hg38_intervals} -wa |uniq > ${dir}/${sample}.mappable.hg38.bed
    bedtools intersect -a ${bed} -b ${tss_intervals} -wa |uniq > ${dir}/${sample}.tss.hg38.bed
