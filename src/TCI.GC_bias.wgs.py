...
map_bed = sys.argv[1]
sample_name = os.path.basename(map_bed).split('.')[0]
out_dir = sys.argv[2]
src_path=os.path.dirname(sys.argv[0])
try:
    os.mkdir(f"{out_dir}/GC_bias")
except:
   pass 
...
genome_GC_frequency = f"{src_path}/../Ref/genome_GC_frequency"
out_dir = out_dir
size_range = [150, 210]
...
in_file = out_dir +'/GC_counts/'+ bed_file_name+'.GC_counts.txt'
print('in_file:',in_file)

#output is smoothed version
smoothed_out_file = out_dir +'/GC_bias/'+ bed_file_name+'.GC_bias.txt'

#plot files
plot_file1 = out_dir +'/GC_plots/'+ bed_file_name+'.GC_bias.pdf'
plot_file2 = out_dir +'/GC_plots/'+ bed_file_name+'.GC_bias.summary.pdf'
plot_file3 = out_dir +'/GC_plots/'+ bed_file_name+'.GC_bias.key_lengths.pdf'
...
for length in range(size_range[0],size_range[1]+1):
    ...
    new_df = pd.concat([new_df, current], ignore_index=True)
...
axes[0,0].set_title(bed_file_name+'\n'+mappable_name + '\n' + old_title)
